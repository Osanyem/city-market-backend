...
import com.market.citymarket.api.exception.CustomerNotFoundException;
import com.market.citymarket.api.exception.GenericAlreadyExistsException;
import com.market.citymarket.api.exception.ItemNotFoundException;
import com.market.citymarket.api.model.Item;
import com.market.citymarket.api.repository.CartRepository;
    ...
    public List<Item> addOrReplaceItemsByCustomerId(String customerId, @Valid Item item) {
        ...
        items.forEach(singleItem -> {
            if (singleItem.getProduct().getId().equals(UUID.fromString(item.getId()))) {
                singleItem.setQuantity(item.getQuantity()).setPrice(singleItem.getPrice());
                itemExists.set(true);
            }
        });
        if (!itemExists.get()) {
